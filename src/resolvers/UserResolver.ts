...
@Resolver((_of) => User)
class UserResolver {
  @Query((_returns) => User, { nullable: true, description: 'Returns a single [User] via a given [uuid]'})
  async getUser(@Arg('uuid') uuid: string) {
    return UserRepo.getUserByUUID(uuid);
  }

  @Mutation((_returns) => User, {description: `Creates a single <User> via given <deviceToken>, <followedPulbications>,
  ...
  @Mutation((_returns) => User, { nullable: true, description: 'Lets the user from a given <uuid> follow the <Publication> given by <pubID>' })
  ...
  @Mutation((_returns) => User, { nullable: true, description: 'Lets the user from a given <uuid> unfollow the <Publication> given by <pubID>' })
  ...
  @Mutation((_returns) => User, { nullable: true, description: 'Adds the <Article> given by the <articleID> to the <User\'s> read articles'})
  ...
  @Mutation((_returns) => User, { nullable: true, description: 'Increments the number of bookmarks for the <User> given by <uuid>'  })
  async bookmarkArticle(@Arg('uuid') uuid: string) {
    const user = await UserRepo.incrementBookmarks(uuid);
    return user;
  }

  @Mutation((_returns) => [User], {description: 'Creates and returns the Weekly Debrief for all users'})
